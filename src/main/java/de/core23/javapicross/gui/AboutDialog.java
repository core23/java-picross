...
import de.core23.javapicross.component.JHyperlink;
import de.core23.javapicross.helper.LanguageManager;
import de.core23.javapicross.misc.ProgramConstants;
...
public class AboutDialog extends JDialog {
	...
	private void initialize() {
		...
		this.setTitle(String.format(LanguageManager.getString("about.title"), "JavaPicross")); //$NON-NLS-1$ //$NON-NLS-2$
	...
	public JPanel getJContentPane() {
		if (_jContentPane == null) {
			...
			JLabel labelVersion = new JLabel("Version " + ProgramConstants.VERSION); //$NON-NLS-1$
			labelVersion.setAlignmentX(Component.CENTER_ALIGNMENT);

			JLabel labelCopyright = new JLabel("Copyright 2011 Christian Gripp"); //$NON-NLS-1$
	...
	public JHyperlink getJHyperlinkPaypal() {
		if (_jHyperlinkPaypal == null) {
			_jHyperlinkPaypal = new JHyperlink(new ImageIcon(getClass().getResource("/icons/paypal.gif")), //$NON-NLS-1$
